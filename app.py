...
def register_routes(app):
    ...
    def verify_local_email_challenge(email, token):
        ...
        if local_verified is not True:
            return None
        user = ensure_local_user(email)
    ...
    def verify_otp():
        ...
        existing_user = User.query.filter_by(username=email).first()
        is_new_account = not existing_user or not existing_user.email_verified
        local_verified_user = verify_local_email_challenge(email, otp)
        if local_verified_user:
            clear_failed_login(email)
            start_authenticated_session(local_verified_user, new_account=is_new_account)
    ...
    def verify_email():
        ...
        if request.method == "POST":
            ...
            if action == "resend":
                ...
                if not form_errors:
                    local_verified_user = verify_local_email_challenge(email, token)
                    if local_verified_user:
                        new_account = bool(session.get("pending_new_account"))
                        clear_failed_login(email)
                        start_authenticated_session(local_verified_user, new_account=new_account)
                        flash("Email verified successfully.", "success")
                        return redirect(url_for("admin_dashboard" if is_admin_user(local_verified_user) else "dashboard"))
                    try:
        ...
        if request.method == "POST":
            ...
            if action == "choose_method":
                    ...
                    if retry_in > 0:
                        ...
                        try:
                            ...
                            if can_use_local_auth_fallback(error):
                                ...
                                session.pop("reset_password_verified", None)
                                remember_otp_request(user.username)
                                flash(f"{local_auth_status_message()} Local reset code: {reset_code}", "warning")
                                form_values["identifier"] = user.username
                                reset_step = "otp"
                            else:
                                form_errors["identifier"] = password_reset_error_message(
                                    error,
                                    "We could not send a reset OTP right now. Please try again later.",
                                )
                                reset_step = "method"
                ...
                if not user or not user.email_verified:
                    ...
                    form_errors["otp"] = "Enter the 6-digit code."
                    reset_step = "otp"
                else:
                    if local_auth_fallback_enabled():
                        local_verified, local_error = verify_local_otp_challenge(user.username, "password_reset", otp)
                ...
                if not user or not user.email_verified:
                    ...
                    if verified:
                        remember_pin_password_reset_verification(user.username)
                        reset_step = "password"
                        flash("Security PIN verified.", "success")
                    else:
                        form_errors["security_pin"] = message or "Verification failed. Please try again."
                        reset_step = "pin"
    ...
    def admin_user_trends(user):
        ...
        cycle_logs = CycleLog.query.filter_by(user_id=user.id).order_by(CycleLog.log_date.desc()).limit(7).all()

        def trend_points(logs, value_getter, max_value):
            points = []
            for log in reversed(logs):
                value = value_getter(log) or 0
                points.append({"label": log.log_date.strftime("%m/%d"), "value": value, "height": max(8, min(100, round((value / max_value) * 100)))})
            return points

        return {
            "sleep": trend_points(lifestyle_logs, lambda log: log.sleep_hours, 10),
            "hydration": trend_points(lifestyle_logs, lambda log: log.water_intake_liters, 4),
            "stress": trend_points(mental_logs, lambda log: log.stress_level, 10),
    ...
    def grouped_admin_notes(limit=30):
        ...
        seen = set()
        for note in notes:
            key = f"{note.admin_id}-{admin_note_excerpt(note.note, 120)}"
            if key in seen:
                continue
            seen.add(key)
            recipient_count = count_records(AdminNote, AdminNote.admin_id == note.admin_id, AdminNote.note == note.note)
            grouped.append(
                {
                    "id": note.id,
                    "note": note.note,
    ...
    def admin_update_note(note_id):
        ...
        target_notes = AdminNote.query.filter(AdminNote.admin_id == note.admin_id, AdminNote.note == original_note_text).all()
        now = datetime.utcnow()
